...
### Fuzzing
The `fuzz/` directory is a detached cargo-fuzz workspace (not a member of the root workspace) with
three targets: `replay_parse` (full `Replay` pipeline), `commands_parse` (raw command stream), and
`shieldbattery_parse` (Sbat section). A committed seed corpus lives in `fuzz/seeds/<target>/`,
derived from `broodrep/testdata/` (regenerate with `cargo run --bin seed_gen` from `fuzz/`; seeds
use a `.seed` extension to avoid the `*.rep` git-lfs rule). CI runs short fuzz jobs on push/PR and
a longer weekly job (`.github/workflows/fuzz.yml`).
```bash
# Requires nightly + cargo-fuzz; run from the repo root
cargo fuzz run replay_parse fuzz/seeds/replay_parse -- -max_len=65536
```
